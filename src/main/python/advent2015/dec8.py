...
in the string data.
"\ x 27" is 6 characters of code, but the string itself contains just one -
...
def encodeLen(s):
    ...
    while i < len(s):
        if (s[i] == '"') or (s[i] == '\\'):
            encSize += 1
        i += 1
    return len(s) + encSize + 2  # surrounding quotes


# part one
total = sum([len(s) for s in content])
mem = sum([memLen(s) for s in content])
enc = sum([encodeLen(s) for s in content])

# print(encodeLen(content[1]))
