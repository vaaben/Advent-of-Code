...
a coordinate pair like 0,0 through 2,2 therefore refers to 9 lights in
...
with open("data/dec6.txt") as f:
    content = f.readlines()

#content = ["turn on 0,0 through 2,2", "turn on 0,0 through 2,2"]
...
for line in content:
    ...
    if m:
        top, left = int(m.group(2)), int(m.group(3))
        bottom, right = int(m.group(4)), int(m.group(5))
        op = m.group(1).strip()
