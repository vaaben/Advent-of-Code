...
or the smallest perimeter of any one face. Each present also requires a
...
# part one
with open("data/dec2.txt") as f:
    content = f.readlines()
    

totalArea = 0
for l in content:
    (l,w,h) = [int(x.strip()) for x in l.split("x")]
    (a,b,c) = (l*w, w*h, h*l)
...
for l in content:
    (l,w,h) = [int(x.strip()) for x in l.split("x")]
    (a,b,c) = (2*l+2*w, 2*w+2*h, 2*h+2*l)
    ribbon = min(a,b,c) + l*w*h
    totalRibbon += ribbon


print("total ribbon "+str(totalRibbon))
