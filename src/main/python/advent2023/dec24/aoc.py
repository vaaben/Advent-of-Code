...
class AoC:
    ...
    def process(self, content, lower, upper, part):
        if part == 1:
            hailstones = [Hailstone(*map(int, line.replace("@", ",").split(","))) for line in content]
            total = 0
            ...
            hailstones = [tuple(map(int, line.replace("@", ",").split(","))) for line in content]
    ...
    def part1(self, filename):
        return self.process(self.read_input(filename), 200000000000000, 400000000000000, 1)

    def part2(self, filename):
        return self.process(self.read_input(filename), 1, 1, 2)
