...
trait Dec3 extends AdventOfCode {
  ...
  def str2number(s: String): Int = {
    s.foldLeft(0)((v, c) => 2 * v + (c.toInt - 48))
  }
...
object Dec3a extends Dec3 with App {
  val entries = inputAsStringList("2021/dec03.txt")

  val bits = entries.head.length - 1
  var gamma = 0
  var epsilon = 0
  (0 to bits).foreach(i => {
    val bitGroups = entries.map(e => e.charAt(i)).groupBy(c => c).maxBy(g => g._2.size)
    val bitValue = bitGroups._1.toInt - 48
    gamma = 2 * gamma + bitValue
    epsilon = 2 * epsilon + (1 - bitValue)
  })
  println(gamma)
  println(epsilon)
...
object Dec3b extends Dec3 with App {
  ...
  val oxygen = (0 to bits).foldLeft(entries)((ents, i) =>
    if(ents.size == 1) {
      ents
    } else {
      val bitGroups = ents.map(e => e.charAt(i)).groupBy(c => c)
        val c = if(bitGroups('0').size == bitGroups('1').size) {
          '1'
        } else {
          bitGroups.maxBy(g => g._2.size)._1
        }

      ents.filter(e => e.charAt(i) == c)
    })

  val co2 = (0 to bits).foldLeft(entries)((ents, i) =>
    if(ents.size == 1) {
      ents
    } else {
      val bitGroups = ents.map(e => e.charAt(i)).groupBy(c => c)
      val c = if(bitGroups('0').size == bitGroups('1').size) {
        ...
        bitGroups.minBy(g => g._2.size)._1
      }
      ents.filter(e => e.charAt(i) == c)
    })

  println(oxygen)
  val o = str2number(oxygen.head)
  println(o)

  println(co2)
  val c = str2number(co2.head)
  println(c)
  println(o*c)
}