...
trait Dec4 extends AdventOfCode {
  ...
  def parseInput(filename: String): (List[Int], List[Board]) = {
    val input = inputAsStringList(filename)

    val numbers = input.head.split(",").map(_.toInt).toList

    val boards = input.tail.filter(_.length > 0).sliding(5,5).map(g => Board(g)).toList
...
class Board(val b: Array[Array[Cell]]) {

  override def toString: String = {
    b.map(r => r.mkString(" ")).mkString("\n")
  }

  def bingo: Boolean = {
    //var bingo = false
    b.exists(a => a.forall(_.checked)) ||
    (0 to 4).exists(i => b.forall(a => a(i).checked))
  }

  def update(number: Int) = {
    b.foreach(_.foreach(c => if(c.number == number) { c.checked = true}))
  }

  def sumUnChecked(): Int = {
    b.map(_.filter(!_.checked).map(_.number).sum).sum
  }
...
object Board {
  ...
  def apply(g : List[String]): Board = {
    val board = new Board(Array.ofDim[Cell](5,5))
    g.zipWithIndex.foreach(a => a._1.split(" ").filter(_.size>0).map(_.toInt).zipWithIndex.foreach(b => board.b(a._2)(b._2) = Cell(b._1, false)) )
    //board.b(0)(0) = Cell(42,false)
    board
  }

}

case class Cell(number: Int, var checked: Boolean) {}
...
object Dec4a extends Dec4 with App {
  val game = parseInput("2021/dec04.txt")

  var last = 0
  game._1.takeWhile(n => {
    last = n
    game._2.foreach(_.update(n))
    !game._2.exists(_.bingo)
  })

  val b = game._2.find(_.bingo).get

  val sum = b.sumUnChecked()
  println(sum)
  println(last)
  println(sum*last)
}
...
object Dec4b extends Dec4 with App {
  ...
  game._1.takeWhile(n => {
    last = n
    val remaining = game._2.filter(!_.bingo)
    if(remaining.size == 1) {
      b = remaining.head
    }
    game._2.foreach(_.update(n))
    !game._2.forall(_.bingo)
  })
