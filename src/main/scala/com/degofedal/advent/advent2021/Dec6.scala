...
trait Dec6 extends AdventOfCode {
  ...
  def evolve(m: Map[Int, Long]): Map[Int,Long] = {
    val raw = m.map(e => (e._1-1, e._2))

    (raw + (6 -> (raw.getOrElse(-1,0L) + raw.getOrElse(6,0L))) + (8 -> raw.getOrElse(-1,0L))).filterNot(_._1 == -1)
  }
...
Realizing what you're trying to do, the submarine automatically produces a list of the ages of several
...
object Dec6a extends Dec6 with App {
  val fishTimes = inputAsIntList("2021/dec06.txt",",")
  val fishMap = fishTimes.groupBy(i => i).map(p => (p._1.toInt, p._2.size.toLong))

  val n = 80

  val fish = (1 to n).foldLeft(fishMap)((map, i) => evolve(map))

  println("fish: "+fish.map(_._2).sum)
