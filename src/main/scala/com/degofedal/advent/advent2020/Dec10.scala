...
object Dec10a extends Dec10 with App {
  val adapters = inputAsIntList("2020/dec10.txt").sorted

  val diffs = (0 :: adapters).sliding(2).foldLeft(mutable.Map[Int,Int]())((m, x) => {
    val diff = x(1)-x(0)
    m.put(diff, m.get(diff).map(_+1).getOrElse(1))
    m
  })

  println(diffs)
  println(diffs.get(1).getOrElse(0)*(diffs.get(3).getOrElse(0)+1))
  // 1920
}
...
object Dec10b extends Dec10 with App {
  ...
  val raw_adapters = inputAsIntList("2020/dec10.txt").sorted

  // switch to array to make indexed lookup faster
  val adapters = (0 :: raw_adapters ::: List(raw_adapters.max+3)).toArray

  val paths = mutable.Map[Int, Long]()
  paths.put(0,1L)

  // for each adapter look at all prior adapters that are within 3 jolages
  // the i'th adapter can be reached through the sum of previous adapters
  Range(1, adapters.size).foreach(i => {
    Range(i-1, -1, -1).foreach(j => {
      if(adapters(i)-adapters(j) <= 3){
        paths.put(i, paths.getOrElse(i,0L) + paths.getOrElse(j,0L))
      }
    })
  })

  // how many paths to the last in line
  println(paths(adapters.size-1))
  // 1511207993344
}