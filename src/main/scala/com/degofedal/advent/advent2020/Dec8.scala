...
trait Dec8 extends AdventOfCode {
  ...
  def simulate(p: List[String]): (Int, Boolean) = {

    var accumulator = 0
    var pointer = 0
    val visited: mutable.ArrayBuffer[Int] = ArrayBuffer()

    def step: Boolean = {
      if(pointer < p.size) {

        val inst = p(pointer).substring(0, 3)
        val arg = p(pointer).substring(4).toInt
...
object Dec8a extends Dec8 with App {
  ...
  val program = inputAsStringList("2020/dec08.txt")

  println(simulate(program))
  // 1723
}
...
object Dec8b extends Dec8 with App {
  ...
  def replaceInstruction(p: List[String], i: Int): List[String] = {
    val oldInst = p(i)
    val newInst = if(oldInst.startsWith("jmp")){
      oldInst.replace("jmp", "nop")
    } else {
      oldInst.replace("nop", "jmp")
    }
    p.take(i) ::: newInst :: p.drop(i+1)
  }

  val mutation = program.zipWithIndex.filter(x => x._1.startsWith("jmp") || x._1.startsWith("nop")).find(x => simulate(replaceInstruction(program, x._2))._2)

  println(mutation)
  mutation.map(m => println(simulate(replaceInstruction(program, m._2))))
  // 846
