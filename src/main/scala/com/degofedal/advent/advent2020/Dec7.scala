...
trait Dec7 extends AdventOfCode {
  ...
  class Bag(val color: String) {
    val content: mutable.ArrayBuffer[(Int, Bag)] = ArrayBuffer()
    val containers: mutable.ArrayBuffer[Bag] = ArrayBuffer()

    override def toString: String = s"color: $color, parents: ${containers.mkString(", ")}"

    def ancestors: Set[Bag] = {

      def inner(b: Bag, l: Set[Bag]): Set[Bag] = {
        if(b.containers.isEmpty) {
          l
        } else {
          b.containers.flatMap(c => inner(c, l + c)).toSet
        }
      }
    ...
    def sumContent: Int = {
      if(content.isEmpty) {
        ...
        content.map(c => c._1 + c._1 * c._2.sumContent).sum
      }
    }

  }

  def toEmptyBag(s: String): Bag = {
    val pattern = """^(.*?) bags.*$""".r
    val pattern(color) = s
    new Bag(color)
  }

  def addContent(s: String, l: List[Bag]): Unit = {
    val contentPattern = """(\d+) (.*?) bag.*""".r
    val pattern = """^(.*?) bags contain (.*)$""".r
    val pattern(color, content) = s

    // find bag
    val bag = l.find(_.color == color).get

    content.split(",").foreach(b => {
      if(b startsWith("no")) {
        // no content - skip
      } else {
        val contentPattern(n, c) = b.trim
        val childBag = l.find(_.color == c).get
        bag.content.append((n.toInt, childBag))
        childBag.containers.append(bag)
      }
    })
  }
...
object Dec7a extends Dec7 with App {
  ...
  val rules = inputAsStringList("2020/dec07.txt")

  val bags = rules.map(toEmptyBag)
  rules.foreach(r => addContent(r, bags))

  val ancestors = bags.find(_.color == "shiny gold").get.ancestors
  //ancestors.foreach(a => println(a.color))
  println(ancestors.size)
  // 144
...
object Dec7b extends Dec7 with App {
  ...
  val sumContent = bags.find(_.color == "shiny gold").get.sumContent
  println(sumContent)
  // 5956
}
