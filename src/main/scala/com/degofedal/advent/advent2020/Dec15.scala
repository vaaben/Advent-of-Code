...
trait Dec15 extends AdventOfCode {

  val state = mutable.Map[Int, Int]() // number, lastSeenRound
 ...
 * the next number to speak is the difference between the turn number when it was last spoken (the previous turn, 4) and
 ...
 */
object Dec15a extends Dec15 with App {

  val inputRaw = "1,17,0,10,18,11,6"

  val input = inputRaw.split(",").map(_.toInt)

  //input.zipWithIndex.foreach(t => state.put(t._1, t._2+1))

  var round = 1
  var seen = -1
  var lastSeen = -1
  do {
    //val lastSeenBefore = state.contains(lastSeen)
    //state.put(lastSeen, round-1)
    if(round <= input.size) {
      seen = input(round-1)
    } else {
      val seenBefore = state.contains(seen)
      if(!seenBefore) {
        seen = 0
      } else {
        seen = round - state.get(lastSeen).get
      }
    }
    println(seen)
    state.put(lastSeen, round)
    lastSeen = seen
    round += 1
  } while (round <= 2020)

  /// last number = solution
  // 595
}
 ...
 */
object Dec15b extends Dec15 with App {
