...
trait Dec9 extends AdventOfCode {

  def isSumInList(s: Long, l: List[Long]): Boolean =
    (for(i <- 0 until l.size; j <- i+1 until l.size) yield (l(i)+l(j))).exists(_ == s)

  def findContinuousSum(s: Long, l: List[Long]): Option[List[Long]] = {

    def inner(s: Long, l: List[Long]): List[Long] = {
      if(l.isEmpty || l.head > s) {
        Nil
      } else if(l.head == s) {
        l.head :: Nil
      } else {
        l.head :: inner(s-l.head, l.tail)
      }
    }

    l.tails.map(m => inner(s, m)).toList.find(x => x.size > 2 && x.sum == s)
  }
}
 ...
 */
object Dec9a extends Dec9 with App {
  val data = inputAsLongList("2020/dec09.txt")

  val invalid = data.sliding(25+1).find(l => !isSumInList(l.last, l.dropRight(1)))
  invalid.map(x => println(x.last))
  // 1504371145
}
 ...
 */
object Dec9b extends Dec9 with App {
  ...
  val solution = findContinuousSum(sum, data)
  println(solution)
  solution.map(x => println(x.min + x.max))
