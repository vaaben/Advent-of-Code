...
trait Dec19 extends AdventOfCode {

  sealed trait Rule
  case class Literal(c: Char) extends Rule
  case class Reference(r: Int) extends Rule
  case class Sequence(l: List[Rule]) extends Rule
  case class Or(a: Rule, b: Rule) extends Rule

  val rulePattern = """^(\d+): (.*)$""".r

  def parseRules(s: String): (Int, Rule) = {
    val rulePattern(num, r) = s.trim

    def inner(s: String): Rule = {
      if(s.startsWith("\"")){
        Literal(s.charAt(1))
      } else if(s.contains("|")) {
        val split = s.split("\\|")
        Or(inner(split(0).trim),inner(split(1).trim))
      } else {
        val split = s.split(" ").map(s => Reference(s.toInt)).toList
        if(split.size == 1) {
          split(0)
        } else {
          Sequence(split)
  ...
  def generate(r: Rule, m: Map[Int, Rule], max: Int = 0): List[String] = {

    def inner(r: Rule): List[String] = {
        r match {
          case a: Literal => List(a.c.toString) // length = 1
          case r: Reference => inner(m.get(r.r).get).filter(_.size < max)
          case s: Sequence =>
            if(s.l.size == 1) {
              inner(s.l(0))
            }else if(s.l.size == 2) {
              inner(s.l(0)).flatMap(a => inner(s.l(1)).map(b => a+b))
            } else {
              inner(s.l(0)).flatMap(a => inner(s.l(1)).flatMap(b => inner(s.l(2)).map(c => a+b+c)))
            }
          case o: Or => inner(o.a) ::: inner(o.b)
        }
    }
    inner(r)
  }
 ...
 *
 * They sent you a list of the rules valid messages should obey and
 * a list of received messages they've collected so far (your puzzle input).
 *
 ...
 * for example, the rule 0: 1 2
 * means that to match rule 0, the text being checked must match rule 1,
 ...
 * then any of the eight options from rule 1, then b (rule 5): aaaabb, aaabab, abbabb, abbbab, aabaab, aabbbb, abaaab, or ababbb.
 *
 ...
 */
object Dec19a extends Dec19 with App {
  val raw=inputAsStringList("2020/dec19.txt")

  val rules = raw.takeWhile(_ != "")
  val messages = raw.drop(rules.size+1)

  val ruleMap = rules.map(parseRules).toMap

  //println(ruleMap)

  //println(ruleMap(2))
  val valid = generate(ruleMap(0), ruleMap)

  //println(valid)

  println(messages.filter(m => valid.contains(m)).size)
  // 230
}
 ...
 */
object Dec19b extends Dec19 with App {
  val raw=inputAsStringList("2020/dec19.txt")

  val rules = raw.takeWhile(_ != "")
  val messages = raw.drop(rules.size+1)

  val ruleMap = rules.map(parseRules).toMap + parseRules("8: 42 | 42 42") + parseRules("11: 42 31 | 200") + parseRules("11: 42 31")

  val longestMessage = messages.maxBy(m => m.size).size

  println(longestMessage)

  //println(ruleMap(2))
  val valid = generate(ruleMap(0), ruleMap, longestMessage+1)

  //println(valid)
  //val pat42 = generate(ruleMap(42), ruleMap, longestMessage+1)
  //val pat31 = generate(ruleMap(31), ruleMap, longestMessage+1)

  val validMessages = messages.filter(m => valid.contains(m))
  //val invalid = messages.filter(m => !valid.contains(m))
