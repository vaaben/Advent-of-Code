...
trait Dec5 extends AdventOfCode {
  ...
  def toSeatId(s: String): Int = {
    s.zipWithIndex.map(c => math.round(math.pow(2, s.size-1-c._2))*map(c._1)).sum.toInt
  }
...
object Dec5a extends Dec5 with App {
  ...
  //println(toSeatId("BBFFBBFRLL"))

  val passes = inputAsStringList("2020/dec05.txt")

  println(passes.map(toSeatId).max)
  // 947
...
object Dec5b extends Dec5 with App {
  ...
  val passes = inputAsStringList("2020/dec05.txt")
  val seat = passes.map(toSeatId).sorted.sliding(2).find(l => l(1)-l(0) == 2)
  println(seat.map(_(0)+1))
  // 636
