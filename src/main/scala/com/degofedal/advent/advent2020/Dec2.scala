...
trait Dec2 extends AdventOfCode {
  ...
  def extractPasswords: List[(Int,Int,Char,String)] = {
    val pwds = inputAsStringList("2020/dec02.txt")

    val pattern = """^(\d+)-(\d+) (.): (.*)$""".r
    pwds.map(s => {
      val pattern(a,b,c,pwd) = s
      (a.toInt,b.toInt,c.charAt(0),pwd)
    })
  }

  def verifyPassword(p : (Int, Int, Char, String)): Boolean = {
    val count = p._4.count(_ == p._3)
    p._1 <= count && count <= p._2
  }

  def verifyPasswordNewStyle(p : (Int, Int, Char, String)): Boolean = {
    p._4.charAt(p._1-1) == p._3 ^ p._4.charAt(p._2-1) == p._3
  }

}

object Dec2a extends Dec2 with App {
  val valid = extractPasswords.filter(verifyPassword)

  println(s"valid passwords: ${valid.size}")
  // valid passwords: 398
}
...
object Dec2b extends Dec2 with App {
  val valid = extractPasswords.filter(verifyPasswordNewStyle)
