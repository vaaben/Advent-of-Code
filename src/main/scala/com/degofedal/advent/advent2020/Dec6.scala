...
trait Dec6 extends AdventOfCode {

  def toCharSet(s : String): Set[Char] = {
    s.toCharArray.toSet
  }

  def distinct(g: List[String]):String = {
    g.foldLeft(Set[Char]())((s, x) => s ++ toCharSet(x)).toList.sorted.mkString("")
  }

  def distinctCount(g: List[String]):Int = {
    g.foldLeft(Set[Char]())((s, x) => s ++ toCharSet(x)).size
  }

  def intersectCount(g: List[String]):Int = {
    g.tail.foldLeft(toCharSet(g.head))((s, x) => s & toCharSet(x)).size
  }
 ...
 */
object Dec6a extends Dec6 with App {

  val rawLines = inputAsStringList("2020/dec06.txt")

  val groups = groupLines(rawLines)
  val sum = groups.map(g => (g, distinctCount(g))).foldLeft(0)((sum, t) => sum + t._2)
 ...
 */
object Dec6b extends Dec6 with App {

  val rawLines = inputAsStringList("2020/dec06.txt")

  val groups = groupLines(rawLines)
  val flaf = groups.map(g => (g, intersectCount(g)))
  flaf.foreach(println)
  val sum = flaf.foldLeft(0)((sum, t) => sum + t._2)
