package com.degofedal.advent.advent2018.dec12
...
/**
  ...
  * Based on the notes, for each generation of plants, a given pot has or does not have a plant based on whether that pot
...
class Dec12 {
  ...
  def applyRules(pots: Array[Char], rules: List[Rule]): Array[Char] = {
    (".." + pots.sliding(5,1).map(w => {
      rules.find(r => r.pattern == w.mkString("")).get.res
    }).mkString("")+"..").toCharArray
  }

}

case class Rule(pattern: String, res: Char)
...
object Dec12a extends App {
  ...
  val offset = 20
  val extra = 130
  val pots = Array.fill[Char](offset+initState.length+extra)('.')

  initState.zipWithIndex.foreach(p => pots(offset + p._2) = p._1)

  val helper = new Dec12

  println(pots.mkString(""))
  //val mut = helper.applyRules(pots, rules)
  val res = (1 to 20).foldLeft(pots)((p,inx) => helper.applyRules(p, rules))
  println(res.mkString(""))

  val sum = res.foldLeft((0,-offset))((sum,p) => if(p == '#') (sum._1 + sum._2, sum._2+1) else (sum._1, sum._2+1) )
  println(sum)

  /*(1 to 120).foreach(k => {
    val res2 = (1 to k).foldLeft(pots)((p,inx) => helper.applyRules(p, rules))
    val sum2 = res2.foldLeft((0,-offset))((sum,p) => if(p == '#') (sum._1 + sum._2, sum._2+1) else (sum._1, sum._2+1) )
    println(res2.mkString("") + sum2 + " " + k)
  })*/

  //val mut = helper.applyRules(pots, rules)

  // after 50000000000 generations
  val f = 9222 + (50000000000L-100)*72
  println(f)
