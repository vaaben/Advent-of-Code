package com.degofedal.advent.advent2018.dec2
...
object Dec2a extends App {

  val input1 = Source.fromResource("2018/Dec2_a.txt").getLines()
  val input2 = Source.fromResource("2018/Dec2_a.txt").getLines()
  val step1 = input1.map(s => s.groupBy(_.toChar)).map(m => m.filter(o => o._2.length ==2)).map(m=>m.map(o => o._2.length)).filter(_.nonEmpty).size
  val step2 = input2.map(s => s.groupBy(_.toChar)).map(m => m.filter(o => o._2.length ==3)).map(m=>m.map(o => o._2.length)).filter(_.nonEmpty).size

  //step2.foreach(println)

  println(step1*step2)

  //val num3 = step1.filter(m => m.find(e => e._2.length==3).isDefined).size

  //println(num3)

  //println(num2.toList.size*num3.toList.size)

  // 6175
}

object Dec2b extends App {

  val input = Source.fromResource("2018/Dec2_a.txt").getLines().toList

  val charDiff = for(
    i <- 0 until input.size-1;
    j <- i+1 until input.size) yield{(input(i),input(j), charDiffCount(input(i),input(j)) )}

  val flaf = charDiff.filter(cd => cd._3 == 1)

  println(flaf)
  flaf.foreach(f => println(commonChars(f._1,f._2)))

  // asgwjcmzredihqoutcylvzinx

  def charDiffCount(a: String, b: String): Int = {
    a.zip(b).foldLeft(0)((count, t) => if(t._1 != t._2) count+1 else count)
  }

  def commonChars(a: String, b: String): String = {
    a.zip(b).foldLeft("")((count, t) => if(t._1 == t._2) count+t._1 else count)
  }
