package com.degofedal.advent.advent2018.dec4

import java.time.LocalDateTime

import com.degofedal.advent.advent2018.dec3.Claim
import com.degofedal.advent.advent2018.dec4.Dec4a.{entries, mostAsleep}
...
class Dec4 extends RegexParsers {
  ...
  def txt: Parser[String] = """[a-zA-Z0-9#\ ]+""".r ^^ { _.toString }

  def entry: Parser[Entry] = "[" ~ int ~ "-" ~ int ~ "-" ~ int ~ int ~ ":" ~ int ~ "]" ~ txt ^^ {case _~y~_~m~_~d~h~_~mi~_~t => Entry(LocalDateTime.of(y,m,d,h,mi),t)}

  def parseEntry(str: String): Entry = {
    val r = parse(entry, str)
    r match {
  ...
  def idFrom(txt: String):Int = {
    val inx = txt.indexOf('#')+1
    txt.substring(inx,txt.indexOf(' ',inx)).toInt
  }

  def perElf(entries: List[Entry]): List[Elf] = {
    val prElf = (entries :+ Entry(LocalDateTime.now(), "Guard #0 begins shift")).foldLeft(((0, List[Entry]()), List[Elf]()))((flaf, entry) => {
      if (entry.txt.contains("#")) {
        // new elf lookup in map
        val newId = idFrom(entry.txt)
        if (flaf._1._2.nonEmpty) {
          ((newId, List()), Elf(flaf._1._1, flaf._1._2) :: flaf._2)
        } else {
          ((newId, List()), flaf._2)
        }
      } else {
        ((flaf._1._1, entry :: flaf._1._2), flaf._2)
      }
    })._2

    prElf.groupBy(_.id).map(e => Elf(e._1, e._2.foldLeft(List[Entry]())((l, elf) => l ::: elf.entries).reverse)).toList
  }

}

case class Entry(ts: LocalDateTime, txt: String)
case class Elf(id: Int, entries: List[Entry]) {

  def minutesAsleep: (Int,Int,Int) = {

    val minMap = Array.ofDim[Int](60)

    entries.sliding(2).foreach(e => increment(e(0).ts.getMinute, e(1).ts.getMinute, minMap))

    (minMap.sum, minMap.indexOf(minMap.max), minMap.max)
  }

  def increment(startMin: Int, endMin: Int, map: Array[Int]): Unit = {
    (startMin until endMin).foreach(i => map(i) = map(i)+1)
  }

}

object Dec4a extends App {
  val dec4 = new Dec4

  val input = Source.fromResource("2018/Dec4.txt").getLines().toList
...
[1518-11-05 00:55] wakes up""".stripMargin.split("\n")*/
  val entries = input.map(l => dec4.parseEntry(l)).sortWith((a,b) => a.ts isBefore(b.ts))

  //entries.foreach(println)

  val mostAsleep = dec4.perElf(entries).map(e => (e.minutesAsleep, e))

  val sleepy = mostAsleep.maxBy(_._1._1)
  println(sleepy)
  println(sleepy._2.id*sleepy._1._2)

}

object Dec4b extends App {
  val dec4 = new Dec4
  ...
  //println(mostAsleep.head)

  val sleepy = mostAsleep.maxBy(_._1._3)
