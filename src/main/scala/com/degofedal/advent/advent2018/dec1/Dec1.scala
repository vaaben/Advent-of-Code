package com.degofedal.advent.advent2018.dec1
...
object Dec1a extends App {

  val result = Source.fromResource("2018/Dec1_a.txt").getLines().foldLeft(0)((sum, str) => sum + str.toInt)
...
object Dec1b extends App {

  val freqs = Source.fromResource("2018/Dec1_a.txt").getLines().map(_.toInt).toList

  var currentFreq: Int = 0
  var seen: Set[Int] = Set()
  var i = 0
  while(!seen.contains(currentFreq)) {
    seen = seen + currentFreq
    currentFreq = currentFreq + freqs(i)
    i = i+1
    if(i == freqs.size){
      i = 0
    }
