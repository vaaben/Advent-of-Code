package com.degofedal.advent.advent2018.dec14
...
class Dec14(e1: Int, e2: Int) {

  var size = 2

  var elf1 = new Entry(e1)(null,null)
  var elf2 = new Entry(e2)(null,null)
  elf1.next = elf2
  elf1.prev = elf2
  elf2.next = elf1
  elf2.prev = elf1
  ...
  def dump: String = {
    (1 to size).foldLeft(first, List[Int]())((l,e) => (l._1.next, l._1.value :: l._2))._2.reverse.mkString("")
  }

  def tail(n: Int): List[Int] = {
    (1 to n).foldLeft(last, List[Int]())((l,e) => (l._1.prev, l._1.value :: l._2))._2
  }

  def add(r: Int): Unit = {
    val newEntry = Entry(r)(null,null)
    newEntry.next = last.next
    newEntry.prev = last
    last.next.prev = newEntry
    last.next = newEntry
    last = newEntry
    size += 1
  }

  def next(e: Entry): Entry = {
    (1 to 1+e.value).foldLeft(e)((a,b) => a.next)
  }

  def tick: Unit = {

    val sum = elf1.value + elf2.value
  ...
  case class Entry(value: Int)(var next: Entry, var prev: Entry)

}

object Dec14pre extends App {

  val n = 2018
  val helper = new Dec14(3,7)
  helper.dump
  while(helper.size < n+10){
    helper.tick
    //helper.dump
    //println(helper.elf1.value +" "+helper.elf2.value)
  }
  println(helper.tail(10).mkString(""))
...
object Dec14a extends App {
  ...
  //println(helper.dump.indexOf(n.toString))
  println(helper.tail(10).mkString(""))

}

object Dec14preb extends App {
  ...
  while(m == -1){
    helper.tick
    m = helper.tail(8).mkString("").indexOf(n.toString)
    //helper.dump
    //println(helper.elf1.value +" "+helper.elf2.value)
  }
  println(helper.dump.indexOf(n.toString))
  //println(m)
  //println(helper.tail10.mkString(""))
