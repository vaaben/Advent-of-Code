 ...
 */
class Dec8(dim: (Int,Int)) extends AdventOfCode {

  val input = inputAsString("2019/dec8a.txt").toCharArray

  val layerSize = dim._1 * dim._2
  val layers = input.sliding(layerSize,layerSize)

}

object Dec8a extends AdventOfCode with App {

  val app = new Dec8(25,6)

  val layerWithFewest0 = app.layers.map(l => (l,l.count(_ == '0'))).minBy(t => t._2)._1
  val grouped = layerWithFewest0.groupBy(c => c)
  val prod12 = grouped('1').size * grouped('2').size

  println(s"Product: $prod12") // 1950
...
object Dec8b extends App {
  ...
  val layers = app.layers.toList

  val flaf = (0 until 25*6).map(i => {
    var l = 0
    while(layers(l)(i) == '2') {
      l += 1
    }
  ...
  flaf.sliding(25,25).foreach(l => println(l.mkString("").replaceAll("1","#").replaceAll("0"," ")))
