 ...
 * 10 starts a new line of output below the current one, and so on.
 ...
 * and ending the list with a newline (ASCII code 10). For example,
 ...
 * the ASCII definitions of the main routine and the movement functions may each contain at most 20 characters,
 ...
 *
 */
class Dec17 extends AdventOfCode {

  val iComp = new IntcodeComputer("2019/dec17.txt")

  var x = 0
  var y = 0
  val scaff = mutable.ListBuffer[(Int,Int)]()
  ...
  def walk: List[String] = {
    ...
    while(!done) {
      vacuumDir match {
        case '^' =>
          if(scaff.contains((vacuumPos._1,vacuumPos._2-1))) {
            moveAhead += 1
            vacuumPos = (vacuumPos._1,vacuumPos._2-1)
          } else {
            if(moveAhead > 0) path.append(s"$moveAhead")
            moveAhead = 0
            if(scaff.contains((vacuumPos._1+1,vacuumPos._2))) {
              path.append("R")
              vacuumDir = '>'
            } else if(scaff.contains((vacuumPos._1-1,vacuumPos._2))) {
              path.append("L")
              vacuumDir = '<'
            } else {
              done = true
        ...
        case '<' =>
          if(scaff.contains((vacuumPos._1-1,vacuumPos._2))) {
            moveAhead += 1
            vacuumPos = (vacuumPos._1-1,vacuumPos._2)
          } else {
            if(moveAhead > 0) path.append(s"$moveAhead")
            moveAhead = 0
            if(scaff.contains((vacuumPos._1,vacuumPos._2-1))) {
              path.append("R")
              vacuumDir = '^'
            } else if(scaff.contains((vacuumPos._1,vacuumPos._2+1))) {
              path.append("L")
              vacuumDir = 'v'
            } else {
              done = true
...
object Dec17a extends App {

  val a = new Dec17

  a.go

  val crossings = a.scaff.filter(p => a.scaff.contains(p._1-1,p._2) && a.scaff.contains(p._1+1,p._2) && a.scaff.contains(p._1,p._2-1) && a.scaff.contains(p._1,p._2+1))
  println(s"Crossings: ${crossings.size}")
  val prodSum = crossings.map(c => c._1*c._2).sum
  println(s"sum of products $prodSum") // 4112
}

object Dec17b extends App {
  ...
  // set input program
  // found manually - what is this alg called?
  val inp = List("C,A,C,B,C,A,B,A,B,A\n", "R,10,R,10,L,4\n","R,4,L,4,L,10,L,10\n","R,10,R,10,R,6,R,4\n","n\n")

  var ascii = inp.flatMap(s => s.toCharArray.map(c => c.toInt))
  //println(ascii)
  a.iComp.input = () => {
    val h = ascii.head
    ascii = ascii.tail
