...
object Dec1a extends AdventOfCode with App {
  ...
  val moduleWeights = Source.fromResource("2019/dec1a.txt").getLines().map(_.toInt).toList

  println(Math.round(moduleWeights.map(m => Math.floor(m/3.0)-2).sum))
  // 3563458
}
...
object Dec1b extends AdventOfCode with App {
  ...
  def calcFuel(n: Int): Int = {
    Math.max(0,Math.floorDiv(n,3) - 2)
  }

  def calcFuel(l: List[Int]): List[Int] = {
    val f = calcFuel(l.head)
    if(f == 0) {
      l
    } else {
      calcFuel(f :: l)
  ...
  println(calcFuel(List(calcFuel(1969))))
  println(calcFuel(List(calcFuel(100756))))

  val moduleWeights = Source.fromResource("2019/dec1a.txt").getLines().map(_.toInt).toList

  val totalFuel = moduleWeights.map(m => calcFuel(List(calcFuel(m))).sum).sum

  println(totalFuel)

  // 16033378 - not correct (includes module weight
  // 5342292 - correct
