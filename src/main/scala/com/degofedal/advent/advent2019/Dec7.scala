...
import com.degofedal.advent.advent2019.Dec7a.{stA, stB, stC, stD, stE}
 ...
 * the second amplifier's output leads to the third amplifier's input, and so on. The first amplifier's input value is 0, and the last amplifier's output leads to your ship's thrusters.
 *
 *     O-------O  O-------O  O-------O  O-------O  O-------O
...
object Dec7a extends AdventOfCode with App {
  ...
  val permutations = List(0,1,2,3,4).permutations

  val testInput = "" //"3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0"

  var stA: mutable.Queue[Long] = mutable.Queue()
  var stB: mutable.Queue[Long] = mutable.Queue()
  var stC: mutable.Queue[Long] = mutable.Queue()
  var stD: mutable.Queue[Long] = mutable.Queue()
  var stE: mutable.Queue[Long] = mutable.Queue()

  val maxOutput =
  permutations.map(t => {
    var res = 0L
    val ampA = new IntcodeComputer("2019/dec7a.txt")
    if(testInput != "")
      ampA.setProgram(testInput)
    ampA.input = () => stA.dequeue()
    ampA.output = (x: Long) => stB.enqueue(x)
    stA.enqueue(t(0))
    stA.enqueue(0)

    val ampB = new IntcodeComputer("2019/dec7a.txt")
    if(testInput != "")
      ampB.setProgram(testInput)
    ampB.input = () => stB.dequeue()
    ampB.output = (x: Long) => stC.enqueue(x)
    stB.enqueue(t(1))

    val ampC = new IntcodeComputer("2019/dec7a.txt")
    if(testInput != "")
      ampC.setProgram(testInput)
    ampC.input = () => stC.dequeue()
    ampC.output = (x: Long) => stD.enqueue(x)
    stC.enqueue(t(2))

    val ampD = new IntcodeComputer("2019/dec7a.txt")
    if(testInput != "")
      ampD.setProgram(testInput)
    ampD.input = () => stD.dequeue()
    ampD.output = (x: Long) => stE.enqueue(x)
    stD.enqueue(t(3))

    val ampE = new IntcodeComputer("2019/dec7a.txt")
    if(testInput != "")
      ampE.setProgram(testInput)
    ampE.input = () => stE.dequeue()
    ampE.output = (x: Long) => res = x
    stE.enqueue(t(4))
  ...
  println(maxOutput) // (List(0, 3, 4, 2, 1),75228)

}

object Dec7b extends AdventOfCode with App {

  val permutations = List(5,6,7,8,9).permutations
  ...
  val maxOutput =
    permutations.map(t => {
      val ampA = new IntcodeComputer("2019/dec7a.txt")
      ampA.name = "A"
      if(testInput != "") ampA.setProgram(testInput)
      ampA.input = () => stA.dequeue()
      ampA.output = (x: Long) => stB.enqueue(x)
      stA.enqueue(t(0))
      stA.enqueue(0)
      ...
      val ampB = new IntcodeComputer("2019/dec7a.txt")
      ampB.name = "B"
      if(testInput != "") ampB.setProgram(testInput)
      ampB.input = () => stB.dequeue()
      ampB.output = (x: Long) => stC.enqueue(x)
      stB.enqueue(t(1))
      ...
      val ampC = new IntcodeComputer("2019/dec7a.txt")
      ampC.name = "C"
      if(testInput != "") ampC.setProgram(testInput)
      ampC.input = () => stC.dequeue()
      ampC.output = (x: Long) => stD.enqueue(x)
      stC.enqueue(t(2))
      ...
      val ampD = new IntcodeComputer("2019/dec7a.txt")
      ampD.name = "D"
      if(testInput != "") ampD.setProgram(testInput)
      ampD.input = () => stD.dequeue()
      ampD.output = (x: Long) => stE.enqueue(x)
      stD.enqueue(t(3))
      ...
      val ampE = new IntcodeComputer("2019/dec7a.txt")
      ampE.name = "E"
      if(testInput != "") ampE.setProgram(testInput)
      ampE.input = () => stE.dequeue()
      ampE.output = (x: Long) => stE.enqueue(x)
      stE.enqueue(t(4))
  ...
  println(maxOutput) // (List(6, 7, 9, 5, 8),79846026)
