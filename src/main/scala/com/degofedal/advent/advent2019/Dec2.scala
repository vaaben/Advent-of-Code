...
class Dec2(input: String) extends AdventOfCode {
  val program: mutable.ListBuffer[Int] = input.split(",").map(_.toInt).toList.to(collection.mutable.ListBuffer)

  def opcode1(a: Int, b: Int, c: Int): Unit = {
    program(c) = program(a) + program(b)
  }

  def opcode2(a: Int, b: Int, c: Int): Unit = {
    program(c) = program(a) * program(b)
  }
  ...
  def run: Int = {
    ...
    while(running) {
      program(index) match {
        case 1  => opcode1(program(index+1),program(index+2),program(index+3))
        case 2  => opcode2(program(index+1),program(index+2),program(index+3))
...
object Dec2a  extends AdventOfCode with App {
  ...
  var prog = new Dec2("1,0,0,0,99")
  prog.run
  println(prog.program)

  prog = new Dec2("2,3,0,3,99")
  prog.run
  println(prog.program)

  prog = new Dec2("2,4,4,5,99,0")
  prog.run
  println(prog.program)

  prog = new Dec2("1,1,1,4,99,5,6,0,99")
  prog.run
  println(prog.program)

  prog = new Dec2(Source.fromResource("2019/dec2a.txt").getLines().mkString)
  prog.program(1) = 12
  prog.program(2) = 2
  println(prog.run)
  // 1114711 - wrong
...
object Dec2b  extends AdventOfCode with App {
  ...
  var res = 0
  while(res != target) {
    val prog = new Dec2(Source.fromResource("2019/dec2a.txt").getLines().mkString)
    prog.program(1) = noun
    prog.program(2) = verb
    res = prog.run
    noun += 1
  ...
  println(s"${noun-1} + $verb")
  println(100 * (noun-1) + verb)
